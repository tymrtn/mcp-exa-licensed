...
function parseStage(v: string | null | undefined, fallback: LicenseStage): LicenseStage {
  if (v === "infer" || v === "embed" || v === "tune" || v === "train") return v;
  if (v === "default") return fallback;
  return fallback;
}

function parseDistribution(v: string | null | undefined, fallback: Distribution): Distribution {
...
export async function licensedFetchText(
  ...
  // Pay via ledger facilitator and retry with licensed_url token
  try {
